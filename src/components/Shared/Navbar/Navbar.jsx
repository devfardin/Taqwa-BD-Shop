...
const Navbar = ({siteData}) => {
  ...
  return (
    <>
            ...
            <div className='grid md:grid-cols-3 grid-cols-2 items-center gap-4'>
              ...
              <div className='flex items-center justify-end gap-3 col-span-1'>
                <div className='hidden md:flex items-center gap-3'>
                  ...
                  <a href={`tel:${siteData.shop_phone}`} className='flex flex-col text-sm hover:text-primary transition-colors'>
                    <span className='text-gray-600'>অর্ডার করতে কল করুন</span>
                    <span className='font-bold text-primary'>{siteData.shop_phone}</span>
                  </a>
                </div>
      ...
      {/* Bottom Menu - Desktop Only - Sticky */}
      <div className='hidden lg:block bg-primary border-t border-gray-200 sticky top-0 z-20'>
        <Container>
          <nav className='flex items-center justify-center py-3'>
            {menuItems.map((item, index) => (
              <Link
                key={index}
                to={item.link}
                className={`px-6 py-1 text-xl font-bold hover:text-menuhover ${
